	      ...
	      )
{
  ...
  for (int TRY=0; TRY<THROW_AWAY+nTry; ++TRY)
	...
	lspmv0.spmvBLAS(1+lastinzero, nVtx, xadj[1+lastinzero+1],
			(uint32_t*)d_xadj0, (uint32_t*)d_adj0, d_val0, 
			d_prin0, d_prout0,
			alpha, beta, stream0);
	...
	lspmv1.spmvBLAS(nVtx-1-lastinzero, nVtx, xadj[nVtx]-xadj[lastinzero+1+1],
			(uint32_t*) (d_xadj1+1+lastinzero), (uint32_t*)d_adj1, d_val1, 
			d_prin1, d_prout1+1+lastinzero,
			alpha, beta,stream1);
