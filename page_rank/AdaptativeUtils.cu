		...
		const int rows_for_vector, int WGSIZE, const bool allocate_row_blocks = true)
{
	...
	//size_t dist2 = std::distance( 150, 10 );
	//std::cout << "rowBlocks " << *rowBlocks << std::endl;


	if (allocate_row_blocks)
	{
		std::cout << "1 rowBlocks=" << *rowBlocks << std::endl;
	...
	//if( (EdgeType)nRows > (EdgeType)std::pow( 2, ROW_BITS ) )
	if( (EdgeType)nRows > 4294967296 )
	{
		
		printf( "Number of Rows in the Sparse Matrix is greater than what is supported at present (%d bits) (%d) (%d) ! ", ROW_BITS, nRows, (EdgeType)pow( 2, ROW_BITS ) );
		return;
	}
	...
	for( i = 1; i <= nRows; i++ )
	{
		...
		sum += row_length;
		//printf("nRows = %d, %d, %d, %d,\n ",nRows, i, row_length, blkSize);
		...
		if( ( i - last_i == 1 ) && sum > blkSize )
		{
			...
			printf("1- %d \n", numWGReq);
			numWGReq = ( numWGReq < 16777216 ) ? numWGReq : 16777216 ;
			printf("2- %d \n", numWGReq);
	...
	if (allocate_row_blocks)
	{
		...
		printf("std::distance(  rowBlocksBase=%d, rowBlocks=%d ) = %d  ", rowBlocksBase, rowBlocks,  dist);
		printf("2*dist = %d <= rowBlockSize = %d ", 2*dist, rowBlockSize);
...
//		std::cout << "rowBlocks[" << i << "]=" << rowBlocks[i] << std::endl;
}
...
void creat_stream(unsigned long *d_rowBlocks, Scalar* alpha, Scalar* beta, Scalar* d_val, EdgeType* d_xadj, VertexType *d_adj, Scalar* d_prin, Scalar* d_prout, unsigned int* d_blkSize, unsigned int* d_rows_for_vector, unsigned int* d_blkMultiplier, tbb::concurrent_bounded_queue<stream_container<VertexType, EdgeType, Scalar>*>* streams, int stream_number ) {
	...
	for(int i=0 ; i < stream_number ; i++ ) {
		...
		cudaError_t cudaErr = cudaStreamCreate( &new_stream );
		cout << "cudaError_t cudaErr = " << cudaErr << endl;
