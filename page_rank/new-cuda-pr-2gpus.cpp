	   ...
	   )
{
	...
	std::cout << "subzie " << subsize <<std::endl;
	...
	for (int TRY=0; TRY<THROW_AWAY+nTry; ++TRY)
	{
		...
		for (int iter = 0; iter < maxiter ; ++ iter) {
			...
			//			std::cout << "------------------------------ number of stream " << streams->size() << std::endl;
			//			std::cout << " ------------------------------ iterr : "<< iter << std::endl;
			...
			while(top < bottom){
				...
				//	std::cout << iteration << " ---  GPU " << current_stream->device << " prend nVtx " << current_stream->m  << " and NNZ  "  << current_stream->nnz << std::endl;
				cudaSetDevice(current_stream->device);
				cusparseSetStream(*current_stream->cusparseHandle, current_stream->stream);

				cusparseStatus0 = cusparseScsrmv(*current_stream->cusparseHandle, CUSPARSE_OPERATION_NON_TRANSPOSE,
						current_stream->m, current_stream->n, current_stream->nnz, current_stream->alpha,
						*current_stream->descr,
