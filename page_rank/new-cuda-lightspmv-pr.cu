	   ...
	   )
{
	...
	vector<lightSpMVCSRKernel> lspmv(stream_number)  ;
	for (int TRY=0; TRY<THROW_AWAY+nTry; ++TRY)
	{
		...
		for (int iter = 0; iter < 40; ++ iter) {
			...
			while(index < nb_tasks){
				...
				/*				cout << "id " << current_stream->id 
								<< ", m : " << current_stream->m 
								<< ", n : "<< current_stream->n 
								<< ", nnz " << current_stream->nnz 
								<< ", stream "<< current_stream->stream <<endl;
				 ...
				 */
				lspmv[current_stream->id].spmvBLAS(current_stream->m, current_stream->n, current_stream->nnz,
